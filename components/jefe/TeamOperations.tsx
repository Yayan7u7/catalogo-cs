...
import { Car, Check, CircleDollarSign, Clock3, MapPin, MessageCircle, Pencil, Repeat2, Search, Send, Smartphone, Star, Upload, UserRoundCheck, UserRoundX, X } from "lucide-react";
...
import type { ConversationMessage, Employee, Service, Trip } from "@/lib/types";

export default function TeamOperations({ initialEmployees, initialServices }: { initialEmployees: Employee[]; initialServices: Service[] }) {
  const [employees, setEmployees] = useState(initialEmployees);
  const [services, setServices] = useState(initialServices);
  const [query, setQuery] = useState("");
  const [historyEmployeeId, setHistoryEmployeeId] = useState("all");
  const [tab, setTab] = useState<"equipo" | "activos" | "historial">("equipo");
  ...
  return <>
    ...
    <div className="mb-6 grid grid-cols-3 gap-2 rounded-xl border border-zinc-800 bg-zinc-950 p-1.5">{([['equipo', 'Disponibilidad'], ['activos', `Activos (${active.length})`], ['historial', 'Historial']] as const).map(([value, label]) => <button key={value} onClick={() => setTab(value)} className={`rounded-lg px-2 py-3 text-[10px] font-semibold uppercase tracking-wider ${tab === value ? "bg-[#C5A55A] text-black" : "text-zinc-500 hover:text-white"}`}>{label}</button>)}</div>
...
function TransportPanel({ service, onRefresh }: { service: Service; onRefresh: () => Promise<void> }) {
  ...
  return <section className="mt-6 space-y-4 border-t border-zinc-800 pt-6"><header><p className="text-[10px] font-semibold uppercase tracking-[0.2em] text-[#C5A55A]">Transporte del servicio</p><p className="mt-1 text-sm text-zinc-500">Gestiona por separado los viajes de ida y regreso.</p></header><div className="grid gap-4 xl:grid-cols-2">{trips.map((trip) => <TripCard key={trip.id} service={service} trip={trip} onRefresh={onRefresh} onRun={run} />)}</div>{service.estadoLiquidacion === "transporte_pendiente" && !trips.some((trip) => trip.tipo === "regreso") && <div className="rounded-xl border border-[#C5A55A]/35 bg-[#C5A55A]/5 p-4"><p className="text-sm font-semibold text-[#E8D5A3]">Transporte de regreso pendiente</p><p className="mt-1 text-xs text-zinc-500">Selecciona cómo regresará la empleada al finalizar el servicio.</p><div className="mt-4 flex flex-wrap gap-2"><ActionButton onClick={() => run(() => chooseReturnTransport(service.id, "chofer"), "Chofer solicitado")}>Regreso con chofer</ActionButton><ActionButton outline onClick={() => run(() => chooseReturnTransport(service.id, "uber"), "Uber seleccionado")}>Regreso con Uber</ActionButton></div></div>}</section>;
}

function TripCard({ service, trip, onRefresh, onRun }: { service: Service; trip: Trip; onRefresh: () => Promise<void>; onRun: (action: () => Promise<{ success: boolean; error?: string }>, success: string) => Promise<void> }) {
  ...
  return <article className="overflow-hidden rounded-xl border border-zinc-800 bg-black"><header className="flex items-center justify-between gap-3 border-b border-zinc-800 px-4 py-3"><div className="flex items-center gap-2"><span className="rounded-lg bg-[#C5A55A]/10 p-2 text-[#C5A55A]"><Car size={17} /></span><div><p className="text-sm font-semibold capitalize">Viaje de {trip.tipo}</p><p className="text-[10px] uppercase tracking-wider text-zinc-600">{trip.proveedorTransporte}</p></div></div><span className="rounded-full border border-zinc-800 px-2.5 py-1 text-[10px] uppercase tracking-wider text-zinc-400">{trip.estado}</span></header>{trip.proveedorTransporte === "uber" ? <div className="space-y-5 p-4"><div className="grid gap-2 sm:grid-cols-2"><a href={buildUberLink(service, trip)} target="_blank" rel="noreferrer" className="rounded-lg border border-[#C5A55A] px-3 py-3 text-center text-xs font-semibold text-[#C5A55A]">Abrir Uber</a><form action={async (data) => { const result = await uploadUberScreenshot(data); result.success ? toast.success("Captura enviada") : toast.error(result.error); }} className="flex min-w-0 items-center gap-2 rounded-lg border border-zinc-800 px-3"><input type="hidden" name="tripId" value={trip.id} /><input required name="file" type="file" accept="image/*" className="min-w-0 flex-1 py-2 text-xs text-zinc-500" /><button className="text-[#C5A55A]" title="Enviar captura"><Upload size={17} /></button></form></div>{trip.estado === "aceptado" && <div className="border-t border-zinc-900 pt-4"><ActionButton outline onClick={() => onRun(() => updateUberStatus(trip.id, "en_camino"), "Uber marcado en camino")}>Uber en camino</ActionButton></div>}{trip.estado === "en_camino" && <div className="border-t border-zinc-900 pt-4"><ActionButton outline onClick={() => onRun(() => updateUberStatus(trip.id, "llegado"), "Llegada notificada")}>Uber llegó</ActionButton></div>}<UberFareEditor trip={trip} onRefresh={onRefresh} />{changeButton}</div> : <div className="space-y-4 p-4"><p className="text-sm text-zinc-500">El viaje será gestionado por un chofer interno.</p>{changeButton}</div>}</article>;
}
...
function UberFareEditor({ trip, onRefresh }: { trip: Trip; onRefresh: () => Promise<void> }) {
  ...
  const canEditFare = !["en_curso", "finalizado", "cancelado"].includes(trip.estado);
...
function buildUberLink(service: Service, trip: Trip) { const ida = trip.tipo === "ida"; const pickupLat = ida ? service.empleada?.ubicacionLat : service.ubicacionClienteLat; const pickupLng = ida ? service.empleada?.ubicacionLng : service.ubicacionClienteLng; const dropLat = ida ? service.ubicacionClienteLat : service.empleada?.ubicacionLat; const dropLng = ida ? service.ubicacionClienteLng : service.empleada?.ubicacionLng; return `https://m.uber.com/ul/?action=setPickup&pickup[latitude]=${pickupLat}&pickup[longitude]=${pickupLng}&dropoff[latitude]=${dropLat}&dropoff[longitude]=${dropLng}`; }
